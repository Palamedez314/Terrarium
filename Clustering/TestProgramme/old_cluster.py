...
def connectedComponents(data_lattice_list, density_dict, tau_eff:int, step_count, eps_bar) -> list[set[tuple[int,...]]]:
    ...
    # tau_distance_set = {lattice_point for lattice_point in tau_box if eff_origin_distance(lattice_point, norm) < tau_eff}
    ...
    # large_boxes = {tuple(map(lambda x: x // 2, point)) for point in surviving_lattice_points}
    # large_box_lattice_dict = {box : set(cartesian_product([list(range(2*comp,2*(comp+1))) for comp in box])) & surviving_lattice_points 
    #                           for box in large_boxes}
    # tau_connection_graph = {}
    # coord_shift = lambda coord : coord-1 if coord > 0 else -coord-1 if coord < 0 else 0
    # sub = lambda x, y : x - y
    ...
    #                             if max(map(coord_shift,map(sub, point, new_point))) < tau_eff}
    #         tau_connection_graph[new_point] = connected_points
    # print("finished alternative box-loop")
